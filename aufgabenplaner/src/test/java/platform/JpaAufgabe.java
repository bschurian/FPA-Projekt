...
/**
 * Erstellen Sie einen JUnit-Testtreiber, der die Operationen des
 * JPA-EntityManager testet. �berpr�fen Sie in Zusicherungen mindestens die
 * Daten auf erwartete Werte, die in dieser Beispielanwendung durch die
 * logger-Aufrufe ausgegeben werden.
 */
public class JpaAufgabe {
	...
	private final String persistenceUnitName = "aufgabenplaner"; // as specified
																	// in
																	// META-INF/persistence.xml
	// createEntityManagerFactory ist eine sehr aufw�ndige Operation! Die
	// EntityManagerFactory muss am Ende manuell geschlossen werden!
	...
	private void execute() throws Exception {
		{ // 1. Entity persistieren in manueller Transaktion mit
			...
			logger.info("contains nach persist: "
					+ entityManager.contains(schritt));
		...
		{ // 2. Wirkung des rollback ausprobieren:
			...
			logger.info("Schritt nach persist: ID=" + idOfFailed + ", titel="
					+ schritt.getTitel());
			...
			logger.info("Schritt nach rollback: ID=" + schritt.getId()
					+ ", titel=" + schritt.getTitel());
			// Macht alle verwalteten Entities DETACHED vom Persistenzkontext:
			entityManager.close();
		...
		{ // 3. Entity mit id holen und �ndern:
			...
			logger.info("Schritt in neuer TX nach find: ID=" + schritt.getId()
					+ ", titel=" + schritt.getTitel() + ", istStunden="
		...
		{ // 4. detach und merge pr�fen:
			...
			schritt.setIstStunden(3);
			{
				...
				tx.em.merge(schritt);
				logger.info("Schritt-IstStunden nach detach, �nderung und merge in neuer TX: "
						+ schritt.getIstStunden());
				tx.close(true);
			}
		}
		...
		{ // 5. Entity wieder mit id holen und pr�fen:
			final EMTransaction tx = new EMTransaction();
			final DmSchritt schritt = tx.em.find(DmSchritt.class, 1L);
			logger.info("Schritt in neuer TX nach close,�ndern,merge,close,find: ID="
					+ schritt.getId()
					+ ", titel="
					+ schritt.getTitel()
					+ ", istStunden=" + schritt.getIstStunden());
		...
		{ // 6. Alle Schritte mit Java Persistence Query Language (JPQL) holen:
			...
			for (final DmSchritt s : results) {
				out.append("Schritt: id=").append(s.getId()).append(", titel=")
						.append(s.getTitel()).append("\n");
			}
		...
		{ // 7. Entity wieder mit id holen und dann l�schen:
			final EMTransaction tx = new EMTransaction();
			final DmSchritt schritt = tx.em.find(DmSchritt.class, 1L);
			logger.info("Schritt in neuer TX vor L�schen: ID="
					+ schritt.getId() + ", titel=" + schritt.getTitel());
			tx.em.remove(schritt);
			logger.info("Schritt in neuer TX nach L�schen: ID="
					+ schritt.getId() + ", titel=" + schritt.getTitel());
			tx.close(true);
		}
